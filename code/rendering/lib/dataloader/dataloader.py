...
class Dataset(data.Dataset):
    ...
    def get_item(self, frame_id):
        img_fpath = os.path.join(self.base_dir, '%08d.png' % (frame_id))
        l1 = self.config.l1
        r1 = self.config.r1
        leng = self.config.leng
        border = 500
        
        img = cv.imread(img_fpath)
        img = cv.copyMakeBorder(img, border, border, border, border, cv.BORDER_CONSTANT, value=0)
        ...
        hand_bbox = self.sample_hand_bbox(skel_map, bbox_size=128)
        return_dict = {
            ...
            'hand_bbox': torch.from_numpy(hand_bbox).long(),
