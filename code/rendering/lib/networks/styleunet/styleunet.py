...
class Discriminator(nn.Module):
    def __init__(self, size, img_channel=6, channel_multiplier=2, blur_kernel=[1, 3, 3, 1]):
        ...
        self.final_linear = nn.Sequential(
            EqualLinear(channels[4] * 4 * 4, channels[4], activation="fused_lrelu"),
            EqualLinear(channels[4], 1, activation="fused_lrelu"),
        )
...
class Discriminator_ori(nn.Module):
...
class ToRGB(nn.Module):
    ...
    def forward(self, input, style, skip=None):
        out = self.conv(input, style)
        #print('input:{} out:{}'.format(input.shape, out.shape))
        out = out + self.bias
...
class Hand_Conv(nn.Module):
    def __init__(self, inp_ch, hid_channel, out_channel, blur_kernel=[1, 3, 3, 1], use_wt=True):
        super().__init__()
        self.conv_in = ConvLayer(inp_ch, hid_channel, 3, downsample=True)
        #self.upsample = nn.ConvTranspose2d(3, 3, kernel_size=4, stride=2, padding=1)
        
        self.upsample = Upsample(blur_kernel)
        self.conv_out = ConvLayer(hid_channel, out_channel, 3, downsample=False)
    ...
    def forward(self, input):
        ...
        out = self.conv_in(input)  # [256, out_channel]
        
        up = self.upsample(out)
        out2 = self.conv_out(up)
        #print('input:{} out:{} up:{} out2:{}'.format(input.shape, out.shape, up.shape, out2.shape))

        return out2
    

class TriPlane_Conv(nn.Module):
    def __init__(self, inp_ch, hid_channel, out_channel, blur_kernel=[1, 3, 3, 1], use_wt=True, recurrence=2):
        super().__init__()
        self.conv_in = ConvLayer(inp_ch, hid_channel, 3, downsample=True)
        self.conv_in2 = ConvLayer(hid_channel, hid_channel, 3, downsample=True)
        self.cca = CrissCrossAttention(hid_channel)
        self.upsample = Upsample(blur_kernel)
        self.upsample2 = Upsample(blur_kernel)
        self.conv_out = ConvLayer(hid_channel, out_channel, 3, downsample=False)
    ...
    def forward(self, input, input2):
        ...
        out2 = self.conv_in(input2)

        out = self.conv_in2(out)  
        out2 = self.conv_in2(out2)
        for i in range(self.recurrence):
            out = self.cca(out, out2)
        up = self.upsample(out)
...
class SWGAN_unet(nn.Module):
    def __init__(self, inp_size, inp_ch, out_ch, out_size, style_dim, n_mlp, middle_size=8, c_dim=0,
        ...
        in_channel = self.channels[inp_size // 2]  
        self.from_rgbs = nn.ModuleList()
        self.cond_convs = nn.ModuleList()
        self.comb_convs = nn.ModuleList()   
        if inp_ch > in_channel * 2:
            self.inp_double = True
            self.conv_in1 = ConvLayer(inp_ch, inp_ch // 2, 3, downsample=True) 
            self.conv_in2 = ConvLayer(inp_ch // 2, in_channel, 3, downsample=False)
        else:
            self.inp_double = False
            self.conv_in = ConvLayer(inp_ch, in_channel, 3, downsample=True)
        for i in range(int(math.log(inp_size, 2)) - 2, self.middle_log_size - 1, -1):  
            out_channel = self.channels[2 ** i]  
            self.from_rgbs.append(FromRGB(in_channel, inp_ch, downsample=True, use_wt=False))  
        ...
        self.face = Hand_Conv(3, 8, 16)
        self.hand = Hand_Conv(5, 8, 16)
    ...
    def forward_face(self, input):
        return self.face(input)
    
    def forward_hand(self, input):
        return self.hand(input)
    ...
    def forward(
        ...
        if forward_hand:
            return self.hand(condition_img)
        
        if forward_face:
            return self.face(condition_img)
        ...
        if self.inp_double:
            ...
            cond_out = self.conv_in2(cond_out)
        else:
