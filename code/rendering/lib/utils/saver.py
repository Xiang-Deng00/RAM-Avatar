...
class CheckpointSaver(object):
    ...
    def load_checkpoint(self, models, optimizers, checkpoint_file=None):
        """Load a checkpoint."""
        if checkpoint_file is None:
            logging.info('Loading latest checkpoint [' + self.latest_checkpoint + ']')
            checkpoint_file = self.latest_checkpoint
        checkpoint = torch.load(checkpoint_file, map_location=torch.device('cpu'))
        for model in models:
            if model in checkpoint:
                if model != 'stylecode' or False:
                    #print('model:', model)
                    models[model].load_state_dict(checkpoint[model], strict=False)
        for optimizer in optimizers:
            if optimizer in checkpoint and False:
    ...
    def get_latest_checkpoint(self):
        ...
        if has_latest:
            checkpoint_list.append(os.path.abspath(os.path.join(self.save_dir, 'latest.pt')))
